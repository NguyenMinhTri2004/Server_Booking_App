...
class VoucherController {
    ...
    getallRateByUserId  = async (req , res , next) => {
        return new SuccessResponse({
            message: "Get Rate Ok",
            metadata : await RateService.get({user : req.query.userId})
        }).send(res)
    };
    ...
    getRateByAccommodation  = async (req , res , next) => {
        return new SuccessResponse({
            message: "Get Rate Ok",
            metadata : await RateService.get({accomodation : req.query.accomodation}),
        }).send(res)
    };
    ...
    updateRate  = async (req , res , next) => {
        return new SuccessResponse({
            ...
            metadata : await RateService.update({rateId : req.body.rateId} , {...req.body})
        }).send(res)
    };
